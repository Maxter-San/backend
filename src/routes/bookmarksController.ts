...
export default async function bookmarksController(req: Request, res: Response) {
    try {
        ...
        res.status(200).send({ "result": "Obtener bookmarks funciona correctamente!", bookmarks });
    }
    catch {
        res.status(500).send({ error: true, "result": "Ocurrió un error durante el getBookmarks." });
